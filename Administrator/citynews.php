...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("News_Uploads/citynewsfolder/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("News_Uploads/citynewsfolder/" . $dyna))
          {
      ...
      "News_Uploads/citynewsfolder/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "News_Uploads/citynewsfolder/" . $_FILES["i_upload"]["name"]);
...
$filetype = "1";
	...
	$table="news_city";
	$columns="created_date,created_time,created_day,created_month,created_year,city,news_title,content,file_type,file_content,admin_status,posted_userid";
	$column_values="'".$date."','".$time."','".$day."','".$month."','".$year."','".$i_city."','".$i_title."','".$i_edit."','".$filetype."','".$dyna."','1','".$post_code."'";
...
alert('successfully inserted');
		window.location.href='citynews.php';
		</script>";
		exit;
}
else
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add City News</div>
	...
	<a href="news_cityreports.php" style="color:white;">View City News Reports </a></span>
	</div>
