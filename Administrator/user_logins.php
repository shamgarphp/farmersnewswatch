...
<html lang="en">
<head>
<title>Farmers Watcher - Users </title> 
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View User Reports</div>
	...
	<a href="add_login_info.php" style="color:white;">  + Add User  </a></span>
	</div>
                    ...
                    <td><?php echo($row['area_name']); ?></td>
                       ...
                       <td><?php echo($row['email']); ?></td>
                          <td><?php echo($row['mobile']); ?></td>
                             <td><?php echo($row['town']." ".$row['city']." ".$row['state']); ?></td>
                             
                                <td>

			<a href="?remove=<?php echo(base64_encode($row['ID'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
...
if(isset($_GET['remove']))
{
	...
	$delsdata=mysql_query("delete from $table $wher");
	...
	if($delsdata)
	{
		...
		echo"<script>
      alert('successfully Deleted');
		window.location.href='user_logins.php';
		</script>";
		exit;
