...
if(isset($_POST['submit']))
{
	...
	if($result)
	{
		...
		window.location.href='admin_course.php?add_edu&succ=".$suc."';
		</script>";
		exit;
	}
	else
	{
		...
		window.location.href='admin_course.php?add_edu&notsucc=".$fail."';
		</script>";
		exit;
	}
}
...
?>
    ...
    <div class="modal-dialog">
      ...
      <input type="text" class="form-control" value="<?php echo($single['course_name']); ?>"  name="edu_type_upd" placeholder="Education Type" pattern="[a-zA-Z+ ]{3,30}" title="Please enter proper education type name" required>
    <input type="hidden" name="hide_id" value="<?php echo($id_modal); ?>" />
        ...
        <div class="modal-footer">
		 <center><input type="submit" class="btn btn-success" name="submit_upd"  value="Update" style="width:100px;margin-top:10px;">

          <button type="button" class="btn btn-danger" data-dismiss="modal" style="width:100px;margin-top:10px;">Close</button>
        </center>
		</div>
      </div>
...
?>
	...
	<div class="col-sm-12" id="edu_type_label">Add Course
	
	<!--<span id="edu_view" style="float:right;font-size:12px;text-shadow:0px 0px 0px red;"><a href="admin_course.php?view_edu"  style="color:#0099FF;font-weight:bold;">View Reports</a></span>-->
	</div>
	<div class="col-sm-12" id="b_content" style="background-color:#F8F8F8;">
		...
		/*	if(isset($_GET['succ']))
			...
			<div class="form-group">
      <label for="usr" id="field_label">Course:</label>
      <input type="text" class="form-control" id="usr" name="edu_type" placeholder="Enter Course" pattern="[a-zA-Z+ ]{3,30}" title="Please enter proper education type name" required>
    </div>
	...
	<div class="form-group">
      ...
      <center><input type="submit" class="btn btn-success" name="submit"  value="Submit" style="width:100px;"></center>
    </div>
...
<?php
	...
	<div class="col-sm-12" id="edu_type_label">Course Reports
	
	<!--<span id="edu_view" style="float:right;font-size:12px;text-shadow:0px 0px 0px red;"><a href="admin_course.php?add_edu" style="color:#0099FF;font-weight:bold;">Add Education Type</a></span>-->
	</div>
...
<?php  echo($inc); ?>
        ...
        <td>
		<a href="admin_course.php?view_edu&edit=<?php echo(base64_encode($row['id'])) ?>" onclick="javascript:if(confirm('Do you want edit')==true){return true}else{ return false;}" title="Edit"><span class="glyphicon glyphicon-edit"></span></a> 
		<a href="admin_course.php?view_edu&remove=<?php echo(base64_encode($row['id'])) ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove">
...
if(isset($_POST['remove_data']))
{
	...
	$sucd=base64_encode("delsucc");
		...
		window.location.href='admin_course.php?view_edu&delsucc=".$sucd."';
		</script>";
		exit;
	}
	else
	{
		...
		window.location.href='admin_course.php?view_edu&delnotsucc=".$failde."';
		</script>";
		exit;
	}
}
...
if(isset($_POST['submit_upd']))
{
	...
	if($updatedata)
	{
		...
		window.location.href='admin_course.php?view_edu';
		</script>";
		exit;
	}
	else
