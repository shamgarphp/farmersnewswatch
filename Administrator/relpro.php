...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("News_Uploads/relatedpromotions/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("News_Uploads/relatedpromotions/" . $dyna))
          {
      ...
      "News_Uploads/relatedpromotions/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "News_Uploads/relatedpromotions/" . $_FILES["i_upload"]["name"]);
...
$filetype = "1";
	...
	$columns="created_date,created_time,created_day,created_month,created_year,news_title,content,video_image_url,file_type,admin_status,posted_userid";
	$column_values="'".$date."','".$time."','".$day."','".$month."','".$year."','".$i_title."','".$i_edit."','".$dyna."','".$filetype."','1','".$post_code."'";
...
alert('successfully inserted');
		window.location.href='relpro.php';
		</script>";
		exit;
}
else
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add Related  promotions</div>
	...
	<a href="news_relreports.php" style="color:white;">View Related  promotions  Reports </a></span>
	</div>
