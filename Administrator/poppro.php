...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("News_Uploads/popularpromotions/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("News_Uploads/popularpromotions/" . $dyna))
          {
      ...
      "News_Uploads/popularpromotions/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "News_Uploads/popularpromotions/" . $_FILES["i_upload"]["name"]);
...
alert('successfully inserted');
		window.location.href='poppro.php';
		</script>";
		exit;
}
else
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add Popular  promotions</div>
	...
	<a href="news_popreports.php" style="color:white;">View Popular  promotions  Reports </a></span>
	</div>
