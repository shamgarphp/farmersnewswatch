...
if(isset($_GET['unap']))
{
    ...
    $qry=mysql_query("update news_world_news set status='0' where id='".$id."' ");
    
    	if($qry)
	{
		echo"<script>
      alert('successfully changed display status into UnApproved');
		window.location.href='magazine_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['ap']))
{
    ...
    $qry=mysql_query("update news_world_news set status='1' where id='".$id."' ");
...
<html lang="en">
<head>
<title>FarmerWatcher - Magazine Administrator </title>
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Magazine Content Reports</div>
	...
	<a href="magazine.php" style="color:white;">  + Add Magazine  </a></span>
	</div>
    ...
    <th style="color:white;">Magazine</th>
	...
	<?php
		...
		$where=" where file_content_pdf!=''  order by category  asc";
		...
		$tab_name="news_world_news";
		$where=" where file_content_pdf!='' and posted_by='".$_SESSION['client']."' order by category  asc";
             ...
             <tr style="text-align:center;">	
				 ...
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['id']) ?>"/>
                  ...
                  <td><?php echo($inc); ?></td>
                    ...
                    <td>
                    <a href="News_Uploads/Worldnews/<?php echo($row['file_content']); ?>" target="_blank"><span class="glyphicon glyphicon-eye-open"></span></a>
                    </td>
...
<!--                  <td>
		<a href="news_worldreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a><br>
				<a href="news_worldreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon glyphicon-edit"></i></a><br>
			<a href="news_worldreports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
                 ...
                 <td>
		<a href="magazine_reports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a><br>
				<a href="magazine_edit.php?edit_edu&edit==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon glyphicon-edit"></i></a><br>
			<a href="magazine_reports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
	...
	if($inc=="0")
		{  	?>
	<tr>
		<td colspan="9" style="text-align:center;color:red;">No Results</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	{
		$ss=mysql_query("select * from news_world_news where id='".$item."' ");
