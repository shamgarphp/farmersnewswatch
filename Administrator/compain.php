...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("News_Uploads/comnpaigningnews/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("News_Uploads/comnpaigningnews/" . $dyna))
          {
      ...
      "News_Uploads/comnpaigningnews/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "News_Uploads/comnpaigningnews/" . $_FILES["i_upload"]["name"]);
...
alert('successfully inserted');
		window.location.href='compain.php';
		</script>";
		exit;
}
else
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add Comnpaigning news</div>
	...
	<a href="news_compainreports.php" style="color:white;">View Comnpaigning reports </a></span>
	</div>
