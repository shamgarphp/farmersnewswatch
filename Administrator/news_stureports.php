...
<div class="modal fade" id="myModal" role="dialog" style=" ">
	...
	<?php		
	$update_tab="news_student_discuss";
    ...
    <div class="row" style="border-radius:5px;border:1px solid lightgrey;padding:10px;">
	  ...
	  <div class="col-sm-12" style="margin-top:10px;border:0px solid green;">		
		...
		if($single['file_type']=="1")
		{
			...
			echo "<a href='News_Uploads/studentdiscussion/".$single['file_content']."' target='_blank'><img style='width:100%;height:200px;' src='News_Uploads/studentdiscussion/".$single['file_content']."' /></a>";
		}
   ...
   <source src="<?php echo("News_Uploads/studentdiscussion/".$single['file_content']); ?>"  >
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View 	student Reports</div>
	...
	<a href="discussion.php" style="color:white;">Add news </a></span>
	</div>
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 ...
                 <td>
		<a href="news_stureports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="news_stureports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
{
	$datas=$_POST['case'];
	$table="news_student_discuss";
	$rv=0;
	foreach($datas as $item)
	{
		$ss=mysql_query("select * from news_student_discuss where id='".$item."' ");
		...
		if($dels)
		{
			unlink("News_Uploads/studentdiscussion/$sr");
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='news_stureports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
   ...
   unlink("News_Uploads/studentdiscussion/$datas2");
