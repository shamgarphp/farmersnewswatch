...
$post_code=strtotime($asd);
	...
	//$i_text=$_POST['i_text'];
		...
		mkdir("../images/Avail_Imgs/avail_products/".$_POST['pid']);
...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("../images/Avail_Imgs/avail_products/".$_POST['pid']."/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("../images/Avail_Imgs/avail_products/".$_POST['pid']."/" . $dyna))
          {
      ...
      "../images/Avail_Imgs/avail_products/".$_POST['pid']."/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "../images/Avail_Imgs/avail_products/".$_POST['pid']."/" . $_FILES["i_upload"]["name"]);
...
$filetype = "1";
	...
	$columns="vPrice,iAviProductId,vProductName,vImage,vAvailabulity,vDescription";
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add Product</div>
	...
	<a href="product_reports.php" style="color:white;">View reports </a></span>
	</div>
	...
	<div class="col-sm-12" id="b_content">
	  ...
	  <?php
	  $qry=mysql_query("select * from available_products where vStatus='Active' ");
	  
	  while($row=mysql_fetch_array($qry))
	  {
		  ?>
		  <option value="<?php echo($row['iAvailableProducts_Id']); ?>"><?php echo($row['name']); ?></option>
		  <?php
	  }
	  ?>
