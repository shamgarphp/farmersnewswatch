        ...
        <div class="modal-body" style="">
 			<div class="row" style="border-radius:5px;border:1px solid lightgrey;padding:10px;">
				...
				<div class="form-group">
					  ...
					  <input type="number" pattern="[0-9]" class="form-control" id="usr" name="number" placeholder="Please Enter Mobile Number"  title="Please Enter Mobile Number" value="<?php echo $row['number']; ?>" required>
				    ...
				    <input type="hidden" name="model_id" value="<?php echo $id_modal; ?>">
...
?>
	...
	<?php
		...
		//  FROM city LEFT JOIN phone_direct ON city.id = phone_direct.city_id where order by asc";
		 ...
		 $sql = "SELECT phone_direct.id,phone_direct.category,phone_direct.con_name,phone_direct.number,city.name FROM phone_direct  INNER JOIN city ON phone_direct.city_id = city.id ";
        }
        else
        {
		// $tab_name="phone_direct";
		// $where=" where posted_by='".$_SESSION['client']."' order by category  asc";
		 $sql = "SELECT phone_direct.id.phone_direct.category,phone_direct.con_name,phone_direct.number,city.name FROM phone_direct  INNER JOIN city ON phone_direct.city_id = city.id where posted_by='".$_SESSION['client']."' ";
        }
...
<!--                    <td><?php echo($inc); ?></td>
                 ...
                 <td>
		<a href="phone_directoryreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon glyphicon-edit"></i></a>
...
<?php
	if(isset($_POST['submited']) && !empty($_POST['submited'])){
		...
		$sql = "UPDATE phone_direct SET category='".$categ."',con_name='".$contact_name."',number='".$numberr."',city_id='".$cityy."' WHERE id='".$model_id."' ";
		print_r($sql);
