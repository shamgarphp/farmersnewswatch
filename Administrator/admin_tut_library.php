...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["imageUpload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("tutions_Uploadsbooks/" . $_FILES["imageUpload"]["name"])) {
        $incre="1";
      $rn= $_FILES["imageUpload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("tutions_Uploadsbooks/" . $dyna))
          {
      ...
      move_uploaded_file($_FILES["imageUpload"]["tmp_name"],
      "tutions_Uploadsbooks/" . $_FILES["imageUpload"]["name"]);
...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["materialUpload".$i]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("tutions_Uploadsbooks/" . $_FILES["materialUpload".$i]["name"])) {
        $incre="1";
      $rn= $_FILES["materialUpload".$i]["name"];
      $dyna1=$incre.$rn;
      //echo $rrn;

      if(file_exists("tutions_Uploadsbooks/" . $dyna1))
          {
      ...
      move_uploaded_file($_FILES["materialUpload".$i]["tmp_name"],
      "tutions_Uploadsbooks/" . $_FILES["materialUpload".$i]["name"]);
...
$filetype = "pdf";
	...
	$month=date('m');
	$year=date('y');
	$sql = "INSERT INTO admin_tut_library (created_date,created_time,Month,Year,class,subj,cover_image,material_image,filetype,material_type)
        VALUES ('".$date."','".$time."','".$month."','".$year."','".$board."','".$class."','".$dyna."','".$dyna1."','".$filetype."','".$mime."')";
		//echo $sql;
		
		//echo "i need a lott".$sql; 
//	$tab_name="admin_tut_library";
//	$columns="created_date,created_time,Month,Year,class,subj,cover_image,material_image,filetype,material_type";
//	$col_values="'".$date."','".$time."','".$month."','".$year."','".$board."','".$class."','".$dyna."','".$dyna1."','".$filetype."','".$mime."'";
...
if($rf==1)
{
	...
	window.location.href='admin_tut_library.php';
	</script>";
	exit;
...
<html lang="en">
<head>
<title>Bslate Education - School Library</title>
...
*/
?>   
   <div class="col-sm-3" id="edu_type_label_user" style="font-size:18px;"> Tutions Library</div>
	<div class="col-sm-9" style="border:0px solid red;text-align:right;">
	<span style="padding-left:10px;"><a href="admin_tut_libraryreports.php" id="sub_labels" style="color:#FF9933;text-shadow:0px 0px 0px;-webkit-text-shadow:0px 0px 0px;-moz-text-shadow:0px 0px 0px;"> View Reports </a></span>
...
<div class="col-sm-12" id="side_label_user" style="padding:8px;margin-top:15px;">
	...
	<div class="col-sm-12" style="text-shadow:0px 0px 10px white;font-size:14px;"> Add Tutions Books / Videos
...
<select class="form-control" name="board" onchange="class_dependency(this.value)" required>
