...
$(function()
{
   ...
   <!--<script type="text/javascript" src="/css/assets/js/plugins/visualization/echarts/echarts.js"></script>-->                        
	...
	<!-------- ------------>
 	...
 	<div class="content" style="padding:40px;margin-top:-100px;border:0px solid red;">
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Bio Product Reports</div>
	...
	<a href="add_bio_products.php" style="color:white;">  + Add   </a></span>
	</div>
	...
	<?php
		...
		$tab_name="abio_produccts_listed_items";
		$where="  order by vProductName  asc";
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 <td>
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['iAvailaBioPL_Id']) ?>"/>
				 </td>
                  <td><?php echo($inc); ?></td>
				                    <td><?php 
									 
											$qr=mysql_query("select * from available_bio_products where iAvailaBioProduct_Id='".$row['iAvailaBioPL_Id']."' ");
											if($rw=mysql_fetch_array($qr))
											{
												echo($rw['vBioProductName']);
											}
											//echo($row['iAviProductId']);
									 
									
									 ?></td>
									 
					<td> <?php 
					echo($row['vProductName']); ?></td>
									 
					<td> <?php 
					echo($row['vPrice']); ?></td>
					
									 
                    <td> <?php echo ($row['vAvailabulity']) ; ?></td>
                 <td> <?php echo (strip_tags($row['vDescription'])) ; ?></td>
                
                   
                    
                   
                 <td>
	 	<a href="bio_product_reports.php?remove=<?php echo(base64_encode($row['iAvailaBioPL_Id'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	foreach($datas as $item)
	{
		...
		$wher="where iAvailaBioPL_Id='".$item."' ";
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='bio_product_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
	...
	$table="abio_produccts_listed_items";
	$wher="where iAvailaBioPL_Id='".$datas1."' ";
