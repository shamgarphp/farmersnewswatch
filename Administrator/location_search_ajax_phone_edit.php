...
if (isset($_POST["method"]) && $_POST["method"] == "district_ajax") {
    ...
    // $query = "SELECT phone_direct.district, district.dist_name FROM phone_direct INNER JOIN district ON phone_direct.district = district.dist_id where phone_direct.id = '".$id_modal."' ";
    // echo $query ; exit;

    $result = mysql_query($query);
    $check = mysql_affected_rows();
    // $row = mysql_fetch_array($result)
    // print_r($row['dist_id']);exit;
    // echo $row['dist_id'];exit;
    if ($check > 0) {?>
          <?php 
           $dist= "SELECT phone_direct.district, district.dist_name FROM phone_direct INNER JOIN district ON phone_direct.district = district.dist_id where phone_direct.id = '".$id_modal."' ";
           // echo $dist ;exit;
           $dist_val= mysql_query($dist);
           $dist_res=mysql_fetch_array($dist_val);
           // print_r($dist_res['district']);exit;
          ?>
...
if (isset($_POST["method"]) && $_POST["method"] == "mandal_ajax") {
    ...
    if ($check > 0) {
        ?>
          ...
          <?php 
           $mand= "SELECT phone_direct.mandal, mandal.mandal_name FROM phone_direct INNER JOIN mandal ON phone_direct.mandal = mandal.mandal_id where phone_direct.id = '".$id_modal."'  ";
           // echo $mand ;exit;
...
if (isset($_POST["method"]) && $_POST["method"] == "village_ajax") {
    ...
    if ($check > 0) {
        ?>
          ...
          <?php 
           $village= "SELECT phone_direct.village, villward.villward_name FROM phone_direct INNER JOIN villward ON phone_direct.village = villward.villward_id where phone_direct.id = '".$id_modal."' ";
           // echo $village ;exit;
