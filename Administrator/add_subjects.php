...
if(isset($_POST['submit']))
{
	...
	$table="add_subjects";
	/*Unique Entry*/
	$conds="where subjects='".$name."' and class='".$ac_reg."'";
	...
	if($testa)
	{
		...
		window.location.href='add_subjects.php?uni=".$un."';
		</script>";
		exit;
	}
	else
	...
	$columns="class,subjects";
	$sevar=$_SESSION['management'];
	$column_values="'".$ac_reg."','".$name."'";
	...
	if($result)
	{
		...
		window.location.href='add_subjects.php?succ=".$suc."';
		</script>";
		exit;
	}
	else
	{
		...
		window.location.href='add_subjects.php?notsucc=".$fail."';
		</script>";
		exit;
	}
...
?>
    ...
    <div class="modal-dialog">
        ...
        <span class="modal-title" style="font-weight:bold;font-size:14px;color:#f8f8f8;">Update Academic Year</span>
        ...
        <label for="usr" id="field_label">Class:</label><br>
        <input type="hidden" class="form-control" name="ssh" value="<?php 
      $ghhsa12=$single['class'];
   $query_s=mysql_query("select * from  add_class where  id='".$ghhsa12."' ORDER By class ASC");
    while($rg=mysql_fetch_array($query_s))
				{
    
                      ?>"   placeholder="Enter class name" pattern="[A-Za-z0-9+ ]{1,30}"  title="Please enter proper class name[EX:BTECH,MTECH...]" required>
      ...
      ?></span>
		...
		<div class="col-sm-10">	  
		<div class="form-group">
        <label for="usr" id="field_label">Subject:</label>
        <input type="text" class="form-control" value="<?php echo($single['subjects']); ?>"  name="edu_type_upd" pattern="[A-Za-z0-9+ ]{1,40}" title="Please enter proper subject name [EX:Telugu]" required />
        <input type="hidden" name="up" value="<?php echo($single['subjects']); ?>" />
    ...
    if(isset($_POST['submit_upd']))
{
	...
	$columns="subjects='".$unname."'";
	$column_values="where id='".$id."'";
	
	
	
	$condsa1=" where subjects='".$unname."' and class='".$das."'";
	...
	window.location.href='add_subjects.php?unii=".$un."';
		</script>";
		exit;
	}
	else
...
exit;
	...
	<div class="col-sm-12" id="divider_label" style="padding:0px;border:0px solid grey;">
	    ...
	    <div id="add_edu_ty">
		<div class="col-sm-3" id="edu_type_label" >Subjects </div>
	...
	<!------ End Insert Alert -------->	
		...
		<div class="form-group">
        <label for="usr" id="field_label">Subject:</label>
        <input type="text" class="form-control" id="usr" name="un_type" placeholder="Enter subject name" pattern="[A-Za-z0-9+ ]{1,40}" title="Please enter proper academic year [EX:2017-2018]" required>
        </div>
	    <div class="form-group">
		...
		$ghh=$row['class'];
				$query_s=mysql_query("select * from add_class where id='".$ghh."' ORDER By class ASC");
				if($rg=mysql_fetch_array($query_s))
				{
					$table_export_data.=" ".$rg['class']." ";
				} 
		       
		$table_export_data.="</td>
		<td>".$row['subjects']."</td>
        <td>
		<a href='add_subjects.php?edit=".base64_encode($row['id'])."' onclick='javascript:if(confirm('Do you want edit')==true){return true}else{ return false;}' title='Edit'><span class='glyphicon glyphicon-edit'></span></a> 
		<a href='add_subjects.php?remove=".base64_encode($row['id'])."' onclick='javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}' title='Remove'>
...
if(isset($_POST['remove_data']))
{
	...
	$sucd=base64_encode("delsucc");
		...
		window.location.href='add_subjects.php?delsucc=".$sucd."';
		</script>";
		exit;
	}
	else
	{
		...
		echo"<script>
		alert('unable to process your request. Please try again later.!');
		window.location.href='add_subjects.php?delnotsucc=".$failde."';
		</script>";
		exit;
	}
}
