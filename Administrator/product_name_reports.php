		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 <td>
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['iAvailableProducts_Id']) ?>"/>
				 </td>
                  <td><?php echo($inc); ?></td>
				                    <td><?php 
                 ...
                 <td>
	 	<a href="product_name_reports.php?remove=<?php echo(base64_encode($row['iAvailableProducts_Id'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	foreach($datas as $item)
	{
		...
		$wher="where iAvailableProducts_Id='".$item."' ";
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='product_name_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
	...
	$table="available_products";
	$wher="where iAvailableProducts_Id='".$datas1."' ";
