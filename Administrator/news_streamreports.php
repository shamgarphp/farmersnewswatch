...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Live Stream Reports</div>
	...
	<a href="stream.php" style="color:white;">Add news </a></span>
	</div>
    ...
    while($rg=mysql_fetch_array($query_s1))
			...
			echo($rg['City']);
                 ...
                 <td>
		<a href="news_streamreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="news_streamreports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	{
		/*$ss=mysql_query("select * from news_stream where id='".$item."' ");
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='news_streamreports.php';
		</script>";
		exit;
	}
	else
