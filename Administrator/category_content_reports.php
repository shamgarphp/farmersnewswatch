...
?>
	...
	<a href="category_content.php" style="color:white;">  + Add   </a></span>
	</div>
	...
	<th style="color:white;">Content Display Type</th>
	...
	<?php
		...
		$where=" where (content_type='1' or content_type='2') order by category  asc";
             ...
             <tr style="text-align:center;">	
				 ...
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['id']) ?>"/>
                  ...
                  <td><?php echo($inc); ?></td>
									...
									$qry=mysql_query("select * from left_categories where iCategories_Id='".$row['category']."' ");
					...
					{
						echo"Direct Content Display";
					}
					else if($row['content_type']=="2")
					{
						echo"Sub Content Wise Display";
					}
					echo ($row['news_title']) ; ?></td>
                 ...
                 <td>
		<a href="category_content_reports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="category_content_reports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='category_content_reports.php';
		</script>";
		exit;
	}
	else
