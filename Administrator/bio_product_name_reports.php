...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Product Name Reports</div>
	...
	<?php
		...
		$tab_name="available_bio_products";
		$where="  order by vBioProductName  asc";
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 <td>
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['iAvailaBioProduct_Id']) ?>"/>
				 </td>
                  <td><?php echo($inc); ?></td>
				                    <td><?php 
									 
											
											echo($row['vBioProductName']);
                 ...
                 <td>
	 	<a href="bio_product_name_reports.php?remove=<?php echo(base64_encode($row['iAvailaBioProduct_Id'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
{
	$datas=$_POST['case'];
	$table="available_bio_products";
	$rv=0;
	foreach($datas as $item)
	{
		 
		$wher="where iAvailaBioProduct_Id='".$item."' ";
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='bio_product_name_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
	...
	$table="available_bio_products";
	$wher="where iAvailaBioProduct_Id='".$datas1."' ";
