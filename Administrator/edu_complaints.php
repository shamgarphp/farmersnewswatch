...
if(isset($_GET['search_remove']))
{
	...
	echo "<script>
	window.location.href='edu_complaints.php';	
	</script>";
    exit;
}
...
<div class="content" style="padding:0px;">
	...
	<div class="col-sm-12" id="edu_type_label" style="padding:10px 0px;"> Search Education institution feedbacks  
	</div>
...
<input type="submit" class="btn btn-success"  name="search" value="Search" style="width:100px;"> 
	...
	$table_export_data.=" <table class='table table-hover' style='border:1px solid lightgrey;'>
	  ...
	  <input type='checkbox' id='selectall'/>
		...
		<th style='font-weight:bold;'>Institute Type</th>
...
$qry_sub=$qry_sub."name='".$name."' and ";	
   ...
   /* if($name!="")
	{
	    $zz=mysql_query("select * from feedback where name='".$name."' and ( usertype='1' or usertype='2' )");
...
$whrs="where ".$thyz." and ( usertype='1' or usertype='2' ) ";	
	
	}
	else
	{
		$whrs="where year='".date('Y')."' and ( usertype='1' or usertype='2' )";	
	}
	...
	if($name!="")
		{
				...
				$query_s=mysql_query("select * from feedback where name='".$name."' and ( usertype='1' or usertype='2' )");
			...
			$ghh=$row['name'];
				...
				if($rg=mysql_fetch_array($query_s))
				{
					...
					$table_export_data_final.=" ".$rg['name']." ";
							...
							if($row['usertype']==1)
							{
							$table_export_data.="<td>College</td>";	
							$table_export_data_final.="<td>College</td>";	
							
							}
							else if($row['usertype']==2)
							{
								$table_export_data.="<td>School</td>";	
							$table_export_data_final.="<td>School</td>";	
...
<a href='edu_complaints.php?view=".base64_encode($row['sno'])."' onclick='javascript:if(confirm('Do you want view')==true){return true}else{ return false;}' title='view'><span class='glyphicon glyphicon-eye-open'></span></a>
<a href='edu_complaints.php?view_edu&remove=".base64_encode($row['sno'])."' onclick='javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}' title='Remove'><span class='glyphicon glyphicon-trash'></span></a>
</td>
</tr>";
...
<td>".$row['postadd']."</td>
	...
	<div class="col-sm-12" style="margin-bottom:10px;padding:0px;height:60px;">
		
<div class="col-sm-4" id="edu_type_label" style="padding:10px 0px;margin-top:25px;">Education Institute Feedbacks Reports

	<a href='edu_complaints.php?search_remove' class="btn btn-info btn-lg" style="padding:0px;">
          <span class="glyphicon glyphicon-refresh" style="font-size:14px;padding:6px;text-align:center;top:-2px;">Refresh</span>
        </a>	

</div>
<div class="col-sm-4" id="edu_type_label" style="border:0px solid green;" >
	<!-------------------delete alert -------------->
...
<!------------- Pagination Code 2 ---------->
		...
		<?php 
				...
				if(isset($_SESSION['search_results']))
				{
					         ...
					         $sql = "SELECT * FROM feedback where ".$_SESSION['search_results']." and ( usertype='1' or usertype='2' )"; 	
				}
				else
				{
                       $sql = "SELECT * FROM feedback where  year='".date('Y')."' and ( usertype='1' or usertype='2' ) "; 
				}
...
if(isset($_GET['view']))
	...
	//$view_sql="select * from clg_quiz_table where sno='".$_GET['view']."'";
	...
	<!-- Modal content-->
        ...
        <span class="modal-title" style="font-weight:bold;font-size:14px;color:#f8f8f8;;">View Education institution Feedback Reports</span>
		...
		<?php
		$update_tab="feedback";
		$conds="where sno='".$id_modal."' and ( usertype='1' or usertype='2' )";
		//echo $conds;
		$vrow=$getsignle->getsinglerecord($update_tab,$conds);
		?>
	    <div class="row">
        <div class="col-sm-12" style="margin-top:15px;border:0px solid black;">
	...
	<div class="col-sm-12" style="margin-top:10px;">
	    <div class="col-sm-3"><b> Message:</b> </div>	
...
if(isset($_POST['remove_data']))
{
	...
	{
		$wher="where sno='".$item."' and ( usertype='1' or usertype='2' )";
...
if(isset($_GET['remove']))
{
	...
	$table="feedback";
	$wher="where sno='".$datas."' and ( usertype='1' or usertype='2' ) ";
