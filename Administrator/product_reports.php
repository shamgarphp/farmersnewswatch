...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Product Reports</div>
	...
	<a href="add_products.php" style="color:white;">  + Add   </a></span>
	</div>
	...
	<?php
		...
		$tab_name="available_products_listed_items";
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 <td>
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['iAvailaPL_Id']) ?>"/>
				 </td>
                  <td><?php echo($inc); ?></td>
				                    <td><?php 
									 
											$qr=mysql_query("select * from available_products where iAvailableProducts_Id='".$row['iAviProductId']."' ");
                 ...
                 <td>
	 	<a href="product_name_reports.php?remove=<?php echo(base64_encode($row['iAvailaPL_Id'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
{
	$datas=$_POST['case'];
	$table="available_products_listed_items";
	$rv=0;
	foreach($datas as $item)
	{
		 
		$wher="where iAvailaPL_Id='".$item."' ";
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='product_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
	...
	$table="available_products_listed_items";
	$wher="where iAvailaPL_Id='".$datas1."' ";
