...
<div class="modal fade" id="myModal" role="dialog" style=" ">
    ...
    <div class="row" style="border-radius:5px;border:1px solid lightgrey;padding:10px;">
	  ...
	  <div class="col-sm-12" style="margin-top:10px;border:0px solid green;">		
		...
		if($single['file_type']=="1")
		{
			...
			echo "<a href='News_Uploads/edunews/".$single['file_content']."' target='_blank'><img style='width:100%;height:200px;' src='News_Uploads/edunews/".$single['file_content']."' /></a>";
		}
   ...
   <source src="<?php echo("News_Uploads/edunews/".$single['file_content']); ?>"  >
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Education Reports</div>
	...
	<a href="edu.php" style="color:white;">Add news </a></span>
	</div>
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 ...
                 <td>
		<a href="news_edureports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="news_edureports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	{
		$ss=mysql_query("select * from news_edu_news where id='".$item."' ");
		...
		if($dels)
		{
			unlink("News_Uploads/edunews/$sr");
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='news_edureports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
   ...
   unlink("News_Uploads/edunews/$datas2");
