...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["imageUpload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("School_Uploadsbooks/" . $_FILES["imageUpload"]["name"])) {
        $incre="1";
      $rn= $_FILES["imageUpload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("School_Uploadsbooks/" . $dyna))
          {
      ...
      move_uploaded_file($_FILES["imageUpload"]["tmp_name"],
      "School_Uploadsbooks/" . $_FILES["imageUpload"]["name"]);
...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["materialUpload".$i]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("School_Uploadsbooks/" . $_FILES["materialUpload".$i]["name"])) {
        $incre="1";
      $rn= $_FILES["materialUpload".$i]["name"];
      $dyna1=$incre.$rn;
      //echo $rrn;

      if(file_exists("School_Uploadsbooks/" . $dyna1))
          {
      ...
      move_uploaded_file($_FILES["materialUpload".$i]["tmp_name"],
      "School_Uploadsbooks/" . $_FILES["materialUpload".$i]["name"]);
...
$filetype = "pdf";
	...
	$columns="created_date,created_time,Month,Year,boardtype,Class,cover_image,material_image,filetype,material_type";
...
if($rf==1)
{
	...
	window.location.href='admin_scl_library.php';
	</script>";
	exit;
...
*/
?>   
   <div class="col-sm-3" id="edu_type_label_user" style="font-size:18px;"> School Library</div>
	<div class="col-sm-9" style="border:0px solid red;text-align:right;">
	<span style="padding-left:10px;"><a href="admin_scl_libraryreports.php" id="sub_labels" style="color:#FF9933;text-shadow:0px 0px 0px;-webkit-text-shadow:0px 0px 0px;-moz-text-shadow:0px 0px 0px;"> View Reports </a></span>
...
<div class="col-sm-12" id="side_label_user" style="padding:8px;margin-top:15px;">
	...
	<div class="col-sm-12" style="text-shadow:0px 0px 10px white;font-size:14px;"> Add School Books / Videos
