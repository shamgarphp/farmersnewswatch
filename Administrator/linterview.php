...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["i_upload"]["error"] > 0) {
   ...
   //   echo $pan_proof_imagename;
    if (file_exists("News_Uploads/liveinterviewnews/" . $_FILES["i_upload"]["name"])) {
        $incre="1";
      $rn= $_FILES["i_upload"]["name"];
      $dyna=$incre.$rn;
      //echo $rrn;

      if(file_exists("News_Uploads/liveinterviewnews/" . $dyna))
          {
      ...
      "News_Uploads/liveinterviewnews/" . $dyna);
                       break;
                      }
      ...
      move_uploaded_file($_FILES["i_upload"]["tmp_name"],
      "News_Uploads/liveinterviewnews/" . $_FILES["i_upload"]["name"]);
...
alert('successfully inserted');
		window.location.href='linterview.php';
		</script>";
		exit;
}
else
...
<div class="" style="padding:0px;">
	...
	<div class="col-sm-9" id="edu_type_label"> Add Live Interview News</div>
	...
	<a href="news_liveinterreports.php" style="color:white;">View Live Interview reports </a></span>
	</div>
