...
?>
	...
	<a href="add_category.php" style="color:white;">  + Add   </a></span>
	</div>
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 <td>
				 <input type="checkbox" class="case" name="case[]" value="<?php echo($row['iCategories_Id']) ?>"/>
				 </td>
                  <td><?php echo($inc); ?></td>
                 ...
                 <td>
		<a href="category_reports.php?view_edu&view==<?php echo(base64_encode($row['iCategories_Id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<!--<a href="category_reports.php?remove=<?php echo(base64_encode($row['iCategories_Id'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>-->
		</td>
		</tr>
...
{
	$datas=$_POST['case'];
	$table="left_categories";
	$rv=0;
	foreach($datas as $item)
	{
		 
		$wher="where  iCategories_Id='".$item."' ";
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='category_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
	...
	$table="left_categories";
	$wher="where iCategories_Id='".$datas1."' ";
