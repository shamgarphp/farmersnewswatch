...
if(isset($_GET['unap']))
{
    ...
    $qry=mysql_query("update phone_dir set status='0' where id='".$id."' ");
    
    	if($qry)
	{
		echo"<script>
      alert('successfully changed display status into UnApproved');
		window.location.href='phone_reports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['ap']))
{
    ...
    $qry=mysql_query("update phone_dir set status='1' where id='".$id."' ");
...
?>
	...
	<?php
		...
		$tab_name="news_world_news";
		 $where=" where posted_by='".$_SESSION['client']."' 
		 by category  asc";
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 ...
                 <td>
		<a href="news_worldreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
...
if(isset($_POST['remove_data']))
{
	...
	{
		$ss=mysql_query("select * from phone_dir where id='".$item."' ");
