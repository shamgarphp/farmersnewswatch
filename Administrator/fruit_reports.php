...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Fruit Crops Reports</div>
	...
	<a href="fruit.php" style="color:white;">  + Add   </a></span>
	</div>
	...
	<?php
		...
		$where=" where category='Fruit' order by id  desc";
		...
		while($row=mysql_fetch_array($sele))
		{
			...
			?>  
             <tr style="text-align:center;">	
                 ...
                 <td>
		<a href="fruit_reports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="fruit_reports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
if(isset($_POST['remove_data']))
{
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='fruit_reports.php';
		</script>";
		exit;
	}
	else
