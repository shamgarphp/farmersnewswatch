...
if(isset($_GET['search_remove']))
{
	...
	echo "<script>
	window.location.href='individual_feedbacks.php';	
	</script>";
    exit;
}
...
<div class="content" style="padding:0px;">
	...
	<div class="col-sm-12" id="edu_type_label" style="padding:10px 0px;"> Search individual student feedbacks  
	</div>
...
$whrs="where ".$thyz." and usertype='200' ";	
	
	}
	else
	{
		$whrs="where year='".date('Y')."' and usertype='200' ";	
	}
	...
	if($name!="")
		{
				...
				$query_s=mysql_query("select * from feedback where name='".$name."' and  usertype='200' ");
...
<a href='individual_feedbacks.php?view=".base64_encode($row['sno'])."' onclick='javascript:if(confirm('Do you want view')==true){return true}else{ return false;}' title='view'><span class='glyphicon glyphicon-eye-open'></span></a>
<a href='individual_feedbacks.php?view_edu&remove=".base64_encode($row['sno'])."' onclick='javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}' title='Remove'><span class='glyphicon glyphicon-trash'></span></a>
</td>
</tr>";
...
<div class="col-sm-4" id="edu_type_label" style="padding:10px 0px;margin-top:25px;">Individual Student Feedbacks Reports

	<a href='individual_feedbacks.php?search_remove' class="btn btn-info btn-lg" style="padding:0px;">
...
<!------------- Pagination Code 2 ---------->
		...
		<?php 
				...
				if(isset($_SESSION['search_results']))
				{
					         ...
					         $sql = "SELECT * FROM feedback where ".$_SESSION['search_results']." and  usertype='200' "; 	
				}
				else
				{
                       $sql = "SELECT * FROM feedback where  year='".date('Y')."' and usertype='200'  "; 
				}
...
if(isset($_GET['view']))
	...
	<!-- Modal content-->
		...
		<?php
		$update_tab="feedback";
		$conds="where sno='".$id_modal."' and usertype='200' ";
		//echo $conds;
...
if(isset($_POST['remove_data']))
{
	...
	{
		$wher="where sno='".$item."' and usertype='200' ";
...
if(isset($_GET['remove']))
{
	...
	$table="feedback";
	$wher="where sno='".$datas."' and usertype='200'  ";
