...
<div class="modal fade" id="myModal" role="dialog" style=" ">
   ...
   <source src="<?php echo("News_Uploads/activitynews/1Yai Yai Yai Iam Your Little Butterfly Song Dance by Playgroup @ LeDauphinz.mp4"); ?>"  >
...
?>
	...
	<div class="col-sm-9" id="edu_type_label" style="padding:0px;font-size:16px;margin-top:30px;">View Live Video Reports</div>
	...
	<a href="lvideo.php" style="color:white;">Add news </a></span>
	</div>
    ...
    while($rg=mysql_fetch_array($query_s1))
			...
			echo($rg['City']);
                 ...
                 <td>
		<a href="news_livevideoreports.php?view_edu&view==<?php echo(base64_encode($row['id'])) ?>" ><i class="glyphicon glyphicon-eye-open"></i></a>
			<a href="news_livevideoreports.php?remove=<?php echo(base64_encode($row['id'])); ?>&file=<?php echo(base64_encode($row['file_content'])); ?>" onclick="javascript:if(confirm('Do you want remove')==true){return true}else{ return false;}" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
		</tr>
...
{
	$datas=$_POST['case'];
	$table="news_live_video";
	$rv=0;
	foreach($datas as $item)
	{
		/*$ss=mysql_query("select * from news_live_video where id='".$item."' ");
		...
		if($dels)
		{
			//unlink("News_Uploads/livevideonews/$sr");
	...
	if($rv==1)
	{
      ...
      alert('successfully Deleted');
		window.location.href='news_livevideoreports.php';
		</script>";
		exit;
	}
	else
...
if(isset($_GET['remove']))
{
   ...
   //unlink("News_Uploads/livevideonews/$datas2");
